  ...
  <f7-page page-content with-subnavbar>
    <f7-navbar>
      <f7-nav-left :back-link="$t('common.label.back')"
                   @back-click="$store.dispatch('DomAir/storeSelectedDomAirSegments')"></f7-nav-left>
      <f7-nav-title>
          ...
          <f7-row no-gap>
            <f7-col width="80">
              ...
              <f7-row class="text-color-gray item-subtitle">
                <f7-col width="100">
                  ...
                  <f7-chip v-if="domAirFlight.corporateAirline==='T'" :text="$t('common.label.corp')"
                           color="blue"/>
                  |
                  {{domAirFlight.flightCarrier}}{{domAirFlight.flightNo}}
                  <f7-chip v-if="domAirFlight.stopNumber!=='False'" :text="$t('air.msg.stopOver')"
                           color="blue"/>
                </f7-col>
              </f7-row>
            </f7-col>
            <f7-col width="20">
              ...
              <f7-row class="text-color-gray text-align-right">
                <f7-col>
                  <f7-icon size="small" v-if="!domAirFlight.showFlightCabins"
          ...
          <f7-row class="item-content no-gap">
            <f7-col width="60">
              ...
              <div style="display: inline-block;">
                <f7-chip v-if="flightCabin.airPriceType==='T'" :text="$t('common.label.corp')"
                         color="blue"/>
                <f7-chip v-if="flightCabin.sysType==='NET'" :text="$t('air.label.Specials')"
                         color="red"/>
                <f7-chip v-if="flightCabin.seatLeft!=='A'" :text="$t('air.label.seatShortage')"
                         color="red"/>
                <f7-chip v-if="flightCabin.interfaceType==='BO'" :text="$t('air.label.sysType')"
                         color="red"/>
              </div>
            </f7-col>
    ...
    <f7-popup class="demo-popup" ref="selectSegment" id="selectSegment">
      <f7-page>
        <f7-navbar>
          ...
          <f7-nav-right>
            <f7-link popup-close="#selectSegment" @click="selectLowestSegment">{{$t('common.label.next')}}
            </f7-link>
          </f7-nav-right>
        </f7-navbar>
        ...
        <f7-list inset class="margin no-flex"
                 ...
                 :key="index">
          <f7-list-item checkbox :checked="defaultSelect === index"
                        @change="selectLowestPriceSegment(segment,index)">
            <div class="item-title" slot="title">
              <f7-row no-gap>
                <f7-col width="75">
                  ...
                  <f7-row class="text-color-gray item-subtitle">
                    <f7-col width="50">
                      {{$i18nMsg(segment.resultSegment.deptCityNameCN,
                      segment.resultSegment.deptCityNameEN)}}
                      {{segment.resultSegment.departTerm}}
                    </f7-col>
                    <f7-col width="50">
                      {{$i18nMsg(segment.resultSegment.arrivalCityNameCN,
                      segment.resultSegment.arrivalCityNameEN)}}
                      {{segment.resultSegment.arrivalTerm}}
                    </f7-col>
                  </f7-row>
                  <f7-row class="text-color-gray item-subtitle">
                    <f7-col width="100">
                      {{$i18nMsg(segment.resultSegment.carrierNameCN,
                      segment.resultSegment.carrierNameEN)}}
                      <f7-chip v-if="segment.resultSegment.corporateAirline==='T'"
                               :text="$t('common.label.corp')" color="blue"/>
                      &nbsp;&nbsp;|&nbsp;&nbsp;
        ...
        <f7-list inset class="margin no-flex">
          <f7-list-item checkbox :checked="defaultSelect === 'A'"
                        @change="selectLowestPriceSegment(selectLowestPrice,'A')">
            <div class="item-title" slot="title">
              <f7-row no-gap>
                <f7-col width="75">
                  <f7-row>
                  ...
                  <f7-row class="text-color-gray item-subtitle">
                    <f7-col width="50">
                      {{$i18nMsg(selectLowestPrice.resultSegment.deptCityNameCN,
                      selectLowestPrice.resultSegment.deptCityNameEN)}}
                      {{selectLowestPrice.resultSegment.departTerm}}
                    </f7-col>
                    <f7-col width="50">
                      {{$i18nMsg(selectLowestPrice.resultSegment.arrivalCityNameCN,
                      selectLowestPrice.resultSegment.arrivalCityNameEN)}}
                      {{selectLowestPrice.resultSegment.arrivalTerm}}
                    </f7-col>
                  </f7-row>
                  <f7-row class="text-color-gray item-subtitle">
                    <f7-col width="100">
                      {{$i18nMsg(selectLowestPrice.resultSegment.carrierNameCN,
                      selectLowestPrice.resultSegment.carrierNameEN)}}
                      <f7-chip v-if="selectLowestPrice.resultSegment.corporateAirline==='T'"
                               :text="$t('common.label.corp')" color="blue"/>
                      &nbsp;&nbsp;|&nbsp;&nbsp;
...
const lastResultDataKey = 'DomAir/lastResultData'
export default {
  name: 'domair-search-last-result',
  data () {
    ...
    this.$processBookEnable(domAirDefaultSetup)
  ...
  methods: {
    ...
    doSearchDomAir () {
      ...
      DomairService.searchDomFlight(searchParam).then(data => {
        ...
        this.$store.dispatch('DomAir/lastResultData', this.resultData)
        this.calcFilterCondition()
  ...
  mounted () {
    if (this[searchParamsKey]) {
      ...
      this.searchDomFlightParam = this.searchDomFlightParams[this.selectedDomAirSegments.length]
